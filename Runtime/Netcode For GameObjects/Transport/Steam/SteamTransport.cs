...
namespace VaporNetcodeForGo.Transport
{
    public class SteamTransport : NetworkTransport
    {
        ...
        {
            if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer) NetworkLog.LogInfoServer(nameof(SteamTransport) + " - DisconnectLocalClient");
        ...
        public override void DisconnectRemoteClient(ulong clientId)
        {
            ...
            if (!connectionMapping.ContainsKey(clientId))
            {
                if (NetworkManager.Singleton.LogLevel <= LogLevel.Error)
                    NetworkLog.LogErrorServer(nameof(SteamTransport) + " - Can't disconect client, client not connected, clientId: " + clientId);
                return;
            }
        ...
        public override ulong GetCurrentRtt(ulong clientId)
        {
            if (isServer)
            {
                ...
                {
                    if (NetworkManager.Singleton.LogLevel <= LogLevel.Error)
                        NetworkLog.LogErrorServer(nameof(SteamTransport) + " - Can't GetCurrentRtt from client, client not connected, clientId: " + clientId);
                }
            }
            else
            {
        ...
        public override void Initialize(NetworkManager networkManager = null)
        {
            ...
            {
                if (NetworkManager.Singleton.LogLevel <= LogLevel.Error)
                    NetworkLog.LogErrorServer(nameof(SteamTransport) + " - Initialize - Steamworks.NET not ready, " + nameof(SteamTransport) + " can not run without it");
                return;
            }
        }
        ...
        public override NetworkEvent PollEvent(out ulong clientId, out ArraySegment<byte> payload, out float receiveTime)
        {
            ...
            while (connectionStatusChangeQueue.Count > 0)
            {
                ...
                if (param.m_info.m_eState == ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_Connecting)
                {
                    ...
                    if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                        NetworkLog.LogInfoServer(nameof(SteamTransport) + " - connection request from " + param.m_info.m_identityRemote.GetSteamID64());
                ...
                {
                    if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                        Debug.Log(nameof(SteamTransport) + " - connection request to " + param.m_info.m_identityRemote.GetSteamID64() + " was accepted!");
                    ...
                    || param.m_info.m_eState == ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_ProblemDetectedLocally)
                {
                    ...
                    if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                        Debug.Log(nameof(SteamTransport) + $" - connection closed for {param.m_info.m_identityRemote.GetSteamID64()} state responce: {param.m_info.m_eState}");
        ...
        public override void Send(ulong clientId, ArraySegment<byte> segment, NetworkDelivery delivery)
        {
            ...
            else
            {
                ...
                if (NetworkManager.Singleton.LogLevel <= LogLevel.Error)
                {
                    ...
                    NetworkLog.LogErrorServer(nameof(SteamTransport.Send) + " - Trying to send on unknown connection: " + clientId);
                }
            }
        }
        ...
        public override bool StartServer()
        {
            ...
            if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                UnityEngine.Debug.Log(nameof(SteamTransport.StartServer));
        ...
        private void CloseP2PSessions()
        {
            ...
            serverUser = null;
            if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                UnityEngine.Debug.Log(nameof(SteamTransport) + " - CloseP2PSessions - has Closed P2P Sessions With all Users");
