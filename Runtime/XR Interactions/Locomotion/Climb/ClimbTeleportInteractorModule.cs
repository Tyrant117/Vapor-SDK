...
namespace VaporXR.Locomotion
{
    ...
    /// </remarks>
    public class ClimbTeleportInteractorModule : InteractorModule
    {
        [SerializeField]
        ...
        void OnClimbEnd(LocomotionProvider provider)
        {
            ...
            {
                case BaseTeleportationInteractable.TeleportTriggerType.OnSelectExited:
                case BaseTeleportationInteractable.TeleportTriggerType.OnSelectEntered:
                    Interactor.StartManualInteraction(m_TargetTeleportVolume.Interactable);
                    Interactor.EndManualInteraction();
                    break;
                case BaseTeleportationInteractable.TeleportTriggerType.OnActivated:
                case BaseTeleportationInteractable.TeleportTriggerType.OnDeactivated:
                    using (m_ActivateEventArgs.Get(out var args))
                    {
                        args.InteractorObject = Interactor;
                        args.InteractableObject = m_TargetTeleportVolume.Interactable;
        ...
        /// <inheritdoc />
        public void GetValidTargets(List<Interactable> targets)
        {
            targets.Clear();
            if (m_TargetTeleportVolume != null)
                targets.Add(m_TargetTeleportVolume.Interactable);
        }

        /// <inheritdoc />
        public bool IsSelectActive => Interactor.IsSelectActive && Interactor.IsPerformingManualInteraction;
        ...
        /// <inheritdoc />
        public void GetActivateTargets(List<Interactable> targets)
        {
            targets.Clear();
            if (m_TargetTeleportVolume != null)
                targets.Add(m_TargetTeleportVolume.Interactable);
