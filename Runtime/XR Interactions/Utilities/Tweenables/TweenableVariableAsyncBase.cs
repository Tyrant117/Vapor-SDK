...
using UnityEngine;
using Vapor.Utilities.NativeCollections;
...
namespace VaporXR.Utilities.Tweenables
{
    ...
    public abstract class TweenableVariableAsyncBase<T> : TweenableVariableBase<T>, IDisposable where T : struct, IEquatable<T>
    {
        ...
        protected override void ExecuteTween(T startValue, T targetValue, float tweenAmount, bool useCurve = false)
        {
            ...
            var stateTransitionIncrement = useCurve ? (byte)math.ceil(tweenAmount * TotalStateTransitionIncrements) : TotalStateTransitionIncrements;
