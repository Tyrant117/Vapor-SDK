...
{
    public class VXRGrabCompositeInteractable : VXRCompositeInteractable, IVXRGrabCompositeInteractable
    {
        ...
        #region Inspector
        protected override bool RequiresHoverInteractable => true;
        protected override bool RequiresSelectInteractable => true;
        ...
        protected override void OnEnable()
        {
            ...
            Select.SelectEntering += OnSelectEntering;
        ...
        private void UpdateTarget(XRInteractionUpdateOrder.UpdatePhase updatePhase, float deltaTime)
        {
            ...
            if (m_ReinitializeDynamicAttachEverySingleGrab && m_GrabCountChanged && m_GrabCountBeforeAndAfterChange.Item2 < m_GrabCountBeforeAndAfterChange.Item1 && Select.InteractorsSelecting.Count == 1 &&
                m_DynamicAttachTransforms.Count > 0 && m_DynamicAttachTransforms.TryGetValue(Select.InteractorsSelecting[0], out var dynamicAttachTransform))
            {
                InitializeDynamicAttachPoseInternal(Select.InteractorsSelecting[0], dynamicAttachTransform);
            }
        ...
        private void OnSelectEntering(SelectEnterEventArgs args)
        {
            ...
            m_GrabCountBeforeAndAfterChange = Select.SelectCountBeforeAndAfterChange;
            ...
            if (Select.InteractorsSelecting.Count == 1)
            {
                Grab();
        ...
        private void OnSelectExiting(SelectExitEventArgs args)
        {
            ...
            if (Select.InteractorsSelecting.Count == 0)
            {
                if (m_ThrowOnDetach)
        ...
        void InvokeGrabTransformersProcess(XRInteractionUpdateOrder.UpdatePhase updatePhase, ref Pose targetPose, ref Vector3 localScale)
        {
            ...
            using (s_ProcessGrabTransformersMarker.Auto())
            {
                ...
                if (grabbed)
                {
                    ...
                    var processed = false;
                    if (hasMultipleGrabTransformer && (Select.InteractorsSelecting.Count > 1 || !CanProcessAnySingleGrabTransformer()))
                    {
                        foreach (var transformer in m_MultipleGrabTransformers.RegisteredSnapshot)
                        {
                            if (!m_MultipleGrabTransformers.IsStillRegistered(transformer))
        ...
        void AddDefaultGrabTransformers()
        {
            ...
            if (m_MultipleGrabTransformers.FlushedCount == 0 && Select.SelectMode == InteractableSelectMode.Multiple && Select.InteractorsSelecting.Count > 1)
        ...
        {
            bool isFirst = Select.InteractorsSelecting.Count <= 1 || ReferenceEquals(attachPoint, Select.InteractorsSelecting[0]);
            ...
            return m_AttachTransform != null ? m_AttachTransform : Select.GetAttachTransform(attachPoint);
        }
        ...
        public void SetTargetPose(Pose pose)
        {
            ...
            m_IsTargetPoseDirty = Select.InteractorsSelecting.Count == 0;
        }
        ...
        protected virtual void InitializeDynamicAttachPose(IVXRSelectInteractor interactor, Transform dynamicAttachTransform)
        {
            ...
            var interactorAttachTransform = interactor.GetAttachTransform(Select);
            ...
            if (matchPosition && ShouldSnapToColliderVolume(interactor) &&
                XRInteractableUtility.TryGetClosestPointOnCollider(Select, position, out var distanceInfo))
            {
        ...
        public void SetTargetLocalScale(Vector3 localScale)
        {
            ...
            // so that the pose is applied in the next phase it is applied.
            m_IsTargetLocalScaleDirty = Select.InteractorsSelecting.Count == 0;
        }
        ...
        private void UpdateCurrentMovementType()
        {
            ...
            for (var index = Select.InteractorsSelecting.Count - 1; index >= 0; --index)
            {
                var baseInteractor = Select.InteractorsSelecting[index];
