...
namespace VaporXR
{
    ...
    public abstract class XRTargetEvaluator : IDisposable
    {
        ...
        [HideInInspector, SerializeField] private XRTargetFilter m_Filter;
        ...
        [HideInInspector, SerializeField, OnValueChanged("OnEnableChanged")]
        bool m_Enabled = true;
        private void OnEnableChanged()
        {
            ...
            bool _IsRuntimeInstance()
            {
#if UNITY_EDITOR
                if (!Application.isPlaying || UnityEditor.PrefabUtility.IsPartOfAnyPrefab(filter))
                    return false;

                var prefabStage = UnityEditor.SceneManagement.PrefabStageUtility.GetCurrentPrefabStage();
                return prefabStage == null || !prefabStage.IsPartOfPrefabContents(filter.gameObject);
#else
                return false;
#endif
            }
        ...
        public bool enabled
        {
            ...
            set
            {
                Debug.AssertFormat(!disposed, $"Trying to change the {nameof(enabled)} property value of a disposed evaluator {GetType().Name}.", filter);
        ...
        public float GetWeightedScore(Interaction.IInteractor interactor, Interactable target)
        {
        ...
        /// <seealso cref="XRTargetFilter"/>
        /// <seealso cref="VXRBaseInteractor.GetValidTargets"/>
        protected abstract float CalculateNormalizedScore(Interaction.IInteractor interactor, Interactable target);
        ...
        public void Dispose()
        {
            Debug.AssertFormat(!disposed, $"Trying to dispose an already disposed evaluator {GetType().Name}", filter);
